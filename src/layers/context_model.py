...
class ContextModel(nn.Module):
    ...
    def __init__(self, num_features, num_phi_features, entropy_model, kernel_size=5):
        ...
        assert isinstance(entropy_model, SymmetricConditional), type(entropy_model)
    ...
    def compress(self, input, condition, return_sym=False):
        ...
        fsymbols = mask_net.pad(fsymbols)
        ...
        for h_idx in range(H):
            for w_idx in range(W):
                patch = mask_net.crop(fsymbols, (h_idx, w_idx))
                patch_phi = mask_net.crop(condition, (h_idx, w_idx), 1)
            
                self._set_condition(patch, patch_phi, padding=False)
                cdf = self.get_cdf(samples)

                symbols = mask_net.crop(patch, windows=1).squeeze() + minmax
        ...
        if return_sym:
            return [ac.flush(), minmax], self.entropy_model.dequantize(mask_net.crop(fsymbols, windows=(H, W))).to(input.device)
        else:
    ...
    def decompress(self, strings, shape, condition):
        ...
        if not isinstance(minmax, int):
            minmax = int.from_bytes(minmax, byteorder='big', signed=False)
        ...
        input = mask_net.pad(torch.zeros(size=shape, device=condition.device))
        ...
        for h_idx in range(H):
            for w_idx in range(W):
                patch = mask_net.crop(input, (h_idx, w_idx))
                patch_phi = mask_net.crop(condition, (h_idx, w_idx), 1)
        ...
        return mask_net.crop(input, windows=(H, W)).to(device)
